...
# Global constant for date parsing
CURRENT_YEAR = datetime.now().year

# --- Top-Level Parsing Functions ---

def parse_unit_price_string(text):
...
def parse_validity_dates(text):
    ...
    try:
        ...
        if range_match:
            ...
            start_date = datetime.strptime(f"{start_str}{CURRENT_YEAR}", "%d.%m.%Y").date()
...
def parse_list_row_offers(soup):
    offers = []
    for discount_row in soup.select('.discount_row'):
        ...
        if unit_price_elem:
            ...
            unit_price, unit = parse_unit_price_string(raw_unit_price)
        ...
        if validity_elem:
            ...
            validity_start, validity_end = parse_validity_dates(validity)
...
def parse_grid_item(discount_div):
    try:
...
def parse_file_worker(filepath):
    """
    Worker function to parse a single file (picklable).
    """
    try:
        ...
        # Method 1: Try .bc_nav (Breadcrumbs)
        ...
        if filename.startswith('sleva_'):
            ...
            offers = parse_list_row_offers(soup)
...
class KupiParser:
    ...
    def run(self, console=None, workers=None):
        ...
        # Filter for specifically .html or .html.gz
        files = [f for f in files if f.endswith('.html') or f.endswith('.html.gz')]
        
        total_files = len(files)
        
        if console:
            console.log(f"Found {total_files} files in {self.data_dir}")
            console.log(f"Starting parallel parsing with {workers or 'default'} workers...")
            console.total = total_files
            console.update(0, "Init...")
        else:
            print(f"Found {total_files} files. Starting parallel parsing...")
        ...
        with concurrent.futures.ProcessPoolExecutor(max_workers=workers) as executor:
            # Submit tasks
            future_to_file = {executor.submit(parse_file_worker, f): f for f in files}
        ...
        if console:
            console.log(f"Parsed {len(self.products)} unique products.")
        else:
        ...
        if console:
            console.log(f"Data saved to {output_path}")
        else:
